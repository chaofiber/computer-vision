...
class Simulator(object):
	...
	def moveParticles(self, particles, uk):
		...
		# TODO: Change this value for the correct one
		prior_particles = np.zeros(len(particles))
		for ii in range(len(particles)):
			noise = self.param.odometry_stdev * np.random.randn()
			prior_particles[ii] = particles[ii] + uk + noise;

			if self.param.circular_hallway:
				prior_particles[ii] = np.mod(prior_particles[ii], self.param.domain)
			else:
				if prior_particles[ii] > self.param.domain:
					prior_particles[ii] = self.param.domain - np.mod(prior_particles[ii], self.param.domain)
				elif prior_particles[ii] < 0:
					prior_particles[ii] *= -1
				else:
					pass
	...
	def resampleParticles(self, prior_particles, particle_weights):
		...
		# normalization:
		resampled_particles = []
		particle_weights = particle_weights / sum(np.array(particle_weights))
		norm_cdf = np.cumsum(particle_weights)
		M = len(prior_particles)
		for m in range(M):
			n = np.random.uniform()
			idx = np.searchsorted(norm_cdf, n)
			resampled_particles.append(prior_particles[min(idx,M-1)])
